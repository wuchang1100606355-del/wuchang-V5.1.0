...
class ResConfigSettings(models.TransientModel):
    ...
    smtp_encryption = fields.Selection([('none','無'),('starttls','STARTTLS'),('ssl','SSL/TLS')], string='加密方式', default='starttls')
    ...
    ai_mode = fields.Selection([
        ...
        ('master_logic','主控邏輯')
    ], string='AI 模式', default='cloud_builtin')
    ...
    def get_values(self):
        ...
        res.update(
            ordering_enabled=(params.get_param('wuchang.ordering_enabled') == 'True'),
            announcement=params.get_param('wuchang.announcement') or '',
            allow_delivery=(params.get_param('wuchang.allow_delivery') == 'True'),
            ...
            webauthn_enabled=(params.get_param('wuchang.webauthn.enabled') == 'True'),
            webauthn_rp_id=params.get_param('wuchang.webauthn.rp_id') or '',
            webauthn_rp_name=params.get_param('wuchang.webauthn.rp_name') or '',
            security_require_2fa=(params.get_param('security.require_2fa') or '').lower() in ('1','true','yes'),
        )
        res.update(
            agent_enabled=(params.get_param('wuchang.agent.enabled') or '').lower() in ('1','true','yes'),
            agent_name=params.get_param('wuchang.agent.name') or '小j',
            agent_email=params.get_param('wuchang.agent.email') or (params.get_param('web.company_email') or 'admin@wuchang.life'),
        )
        res.update(
            ...
            body_enabled=(params.get_param('wuchang.body.enabled') or '').lower() in ('1','true','yes'),
            body_name=params.get_param('wuchang.body.name') or '洛地',
            body_location=params.get_param('wuchang.body.location') or '',
            voice_enabled=(params.get_param('wuchang.voice.enabled') or '').lower() in ('1','true','yes'),
            asr_host=params.get_param('wuchang.asr.host') or 'asr.wuchang.life',
            tts_host=params.get_param('wuchang.tts.host') or 'tts.wuchang.life',
            llm_host=params.get_param('wuchang.llm.host') or 'llm.wuchang.life',
        )
        res.update(
            ws_privacy_enabled=(params.get_param('wuchang.ws_privacy.enabled') or '').lower() in ('1','true','yes'),
            ws_privacy_anonymize_logs=(params.get_param('wuchang.ws_privacy.anonymize_logs') or '').lower() in ('1','true','yes'),
            ws_privacy_retention_days=int(params.get_param('wuchang.ws_privacy.retention_days') or '7'),
            ws_privacy_disable_analytics=(params.get_param('wuchang.ws_privacy.disable_analytics') or '').lower() in ('1','true','yes'),
        )
        try:
    ...
    def set_values(self):
        super().set_values()
        params = self.env['ir.config_parameter'].sudo()
        params.set_param('wuchang.ordering_enabled', 'True' if self.ordering_enabled else 'False')
        params.set_param('wuchang.announcement', self.announcement or '')
        params.set_param('wuchang.allow_delivery', 'True' if self.allow_delivery else 'False')
        params.set_param('wuchang.min_amount', str(self.min_amount or 0))
        params.set_param('wuchang.kb.enabled', 'True' if self.kb_enabled else 'False')
        params.set_param('wuchang.kb.plaque', self.kb_plaque_text or '五常史料馆')
        params.set_param('wuchang.webauthn.enabled', 'True' if self.webauthn_enabled else 'False')
        params.set_param('wuchang.webauthn.rp_id', self.webauthn_rp_id or '')
        params.set_param('wuchang.webauthn.rp_name', self.webauthn_rp_name or '')
        params.set_param('security.require_2fa', 'True' if self.security_require_2fa else 'False')
        params.set_param('wuchang.agent.enabled', 'True' if self.agent_enabled else 'False')
        params.set_param('wuchang.agent.name', self.agent_name or '小j')
        params.set_param('wuchang.agent.email', self.agent_email or '')
        params.set_param('wuchang.ai_mode', self.ai_mode or 'cloud_builtin')
        ...
        params.set_param('wuchang.body.enabled', 'True' if self.body_enabled else 'False')
        params.set_param('wuchang.body.name', self.body_name or '洛地')
        params.set_param('wuchang.body.location', self.body_location or '')
        params.set_param('wuchang.voice.enabled', 'True' if self.voice_enabled else 'False')
        params.set_param('wuchang.asr.host', self.asr_host or 'asr.wuchang.life')
        params.set_param('wuchang.tts.host', self.tts_host or 'tts.wuchang.life')
        params.set_param('wuchang.llm.host', self.llm_host or 'llm.wuchang.life')
        params.set_param('wuchang.ws_privacy.enabled', 'True' if self.ws_privacy_enabled else 'False')
        params.set_param('wuchang.ws_privacy.anonymize_logs', 'True' if self.ws_privacy_anonymize_logs else 'False')
        params.set_param('wuchang.ws_privacy.retention_days', str(int(self.ws_privacy_retention_days or 0) or 7))
        params.set_param('wuchang.ws_privacy.disable_analytics', 'True' if self.ws_privacy_disable_analytics else 'False')
        try:
            if self.agent_enabled and self.agent_email:
                Partner = self.env['res.partner'].sudo()
                p = Partner.search([('email', '=', self.agent_email)], limit=1)
                vals = {'name': self.agent_name or '小j', 'email': self.agent_email}
                if p:
                    p.write(vals)
                else:
                    Partner.create(vals)
        ...
        try:
            ...
            existing = None
            if self.smtp_host:
                existing = MailServer.search([('smtp_host', '=', self.smtp_host)], limit=1)
            if existing:
                existing.write(vals)
            else:
