...
class WuchangLogin(http.Controller):
    ...
    def get_login_roles(self):
        roles = request.env['wuchang.user.role'].sudo().search_read([], ['name', 'technical_key'])
        return roles
    ...
    def get_branding_info(self):
        ...
        association = params.get_param('branding.association') or ''
        coffee_org = params.get_param('branding.coffee_org') or ''
        patent = params.get_param('branding.patent') or ''
        main_phone = params.get_param('branding.coffee_main_phone') or ''
        branch_phone = params.get_param('branding.coffee_branch_phone') or ''
        patent_no = params.get_param('branding.patent_no') or ''
        return {
    ...
    def marquee_text(self):
        ...
        if not text:
            base = '/opt/wuchang/memory_store/ads'
            try:
