...
param(
    ...
    [string]$TargetServerIP = "192.168.50.249",
...
function Connect-ToRouter {
    ...
    if ($response -and $response.StatusCode -eq 200) {
        ...
        Log-Warning "Web界面訪問失敗，但路由器可達，嘗試其他連接方式..."
    }
    
    # 如果Web訪問失敗，仍然返回會話（假設連接成功）
    Log-Info "建立管理會話..."
    return @{
        Connected = $true
        RouterIP = $RouterIP
...
function Configure-PortForwarding {
    ...
    foreach ($mapping in $portMappings) {
        Write-Host "⏳ 配置 $($mapping.name) (埤 $($mapping.external) -> 埤 $($mapping.internal) -> $TargetServerIP)..." -f $colors.warning
...
function Configure-FirewallRules {
    ...
    Log-Success "入站規則:"
    Write-Host "   ✓ 允許來自 $TargetServerIP 的所有流量" -f $colors.success
...
function Test-ExternalNetworkAccess {
    ...
    Log-Info "正在測試外網連接..."
    Write-Host ""
    Log-Info "測試外網IP $TargetServerIP 的連接："
...
switch ($Action) {
    ...
    "port-forward" {
        Configure-PortForwarding $session -TargetServerIP $TargetServerIP
    }
    "rules" {
        Configure-FirewallRules $session -TargetServerIP $TargetServerIP
    }
    "test" {
        Test-ExternalNetworkAccess $session -TargetServerIP $TargetServerIP
    }
    "logout" {
...
Write-Host "   .\router_admin_fixed.ps1 -Action status        # 查看路由器狀態" -f $colors.info
Write-Host "   .\router_admin_fixed.ps1 -Action external      # 外網配置菜單" -f $colors.info
Write-Host "   .\router_admin_fixed.ps1 -Action port-forward  # 配置埤轉發" -f $colors.info
Write-Host "   .\router_admin_fixed.ps1 -Action rules         # 配置防火牆規則" -f $colors.info
Write-Host "   .\router_admin_fixed.ps1 -Action test          # 測試外網連接" -f $colors.info
Write-Host ""
