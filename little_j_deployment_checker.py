...
def log(message: str, level: str = "INFO"):
    ...
    # 同時寫入日誌檔案
    log_file = LOGS_DIR / f"deployment_check_{datetime.now().strftime('%Y%m%d')}.log"
...
def save_report(report: Dict):
    ...
    # JSON 報告
    json_file = REPORTS_DIR / f"deployment_status_{timestamp}.json"
    with open(json_file, 'w', encoding='utf-8') as f:
        json.dump(report, f, indent=2, ensure_ascii=False)
    
    # Markdown 報告
    md_file = REPORTS_DIR / f"deployment_status_{timestamp}.md"
    md_content = generate_markdown_report(report)
    with open(md_file, 'w', encoding='utf-8') as f:
        f.write(md_content)
    
    # 最新報告（覆蓋）
    latest_json = REPORTS_DIR / "deployment_status_latest.json"
    with open(latest_json, 'w', encoding='utf-8') as f:
        json.dump(report, f, indent=2, ensure_ascii=False)
    
    latest_md = REPORTS_DIR / "deployment_status_latest.md"
