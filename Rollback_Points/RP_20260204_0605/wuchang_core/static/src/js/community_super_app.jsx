...
import { 
  ...
  Truck, ClipboardList, Trash2, Minus, Flame, Smile, ThumbsUp, Zap, Droplets, Wifi
} from 'lucide-react';
import { 
    ...
    <div className="flex flex-col h-full bg-slate-50">
        ...
        <div className="p-4 space-y-4">
             ...
             {globalState.fundTransactions.map(tx => (
                 <div key={tx.id} className="bg-white p-3 rounded-lg border border-slate-100 flex justify-between text-sm">
                     <span>{tx.desc}</span>
    ...
    return (
        <div className="flex justify-center bg-gray-200 h-screen font-sans">
            <div className="w-full max-w-md bg-gray-50 h-full flex flex-col relative overflow-hidden shadow-2xl">
                ...
                {tab === 'home' && (
                    <div className="flex-1 overflow-y-auto pb-24">
                        ...
                        <div className="px-4 mb-6 grid grid-cols-2 gap-3">
                             <button onClick={() => onNavigate('volunteer')} className="bg-orange-50 p-4 rounded-xl flex items-center gap-3 shadow-sm border border-orange-100">
                             ...
                             <button onClick={() => onNavigate('wishing')} className="bg-pink-50 p-4 rounded-xl flex items-center gap-3 shadow-sm border border-pink-100">
                             ...
                             <button onClick={() => onNavigate('market')} className="bg-indigo-50 p-4 rounded-xl flex items-center gap-3 shadow-sm border border-indigo-100">
                             ...
                             <button onClick={() => onNavigate('fund')} className="bg-emerald-50 p-4 rounded-xl flex items-center gap-3 shadow-sm border border-emerald-100">
                ...
                {/* BOTTOM NAVIGATION */}
                <div className="bg-white border-t h-20 flex justify-around items-start pt-3 absolute bottom-0 w-full z-20">
                    <button onClick={()=>setTab('home')} className={`flex flex-col items-center w-16 ${tab==='home'?'text-blue-600':'text-gray-400'}`}><Home className="w-6 h-6"/><span className="text-[10px] mt-1">首頁</span></button>
                    <button onClick={()=>onNavigate('market')} className={`flex flex-col items-center w-16 text-gray-400`}><Store className="w-6 h-6"/><span className="text-[10px] mt-1">市集</span></button>
                    <button onClick={()=>alert('開發中')} className={`flex flex-col items-center w-16 text-gray-400`}><MessageCircle className="w-6 h-6"/><span className="text-[10px] mt-1">聊天</span></button>
                    <button onClick={()=>setTab('wallet')} className={`flex flex-col items-center w-16 ${tab==='wallet'?'text-blue-600':'text-gray-400'}`}><Wallet className="w-6 h-6"/><span className="text-[10px] mt-1">錢包</span></button>
                    <button onClick={()=>alert('開發中')} className={`flex flex-col items-center w-16 text-gray-400`}><User className="w-6 h-6"/><span className="text-[10px] mt-1">我的</span></button>
                </div>
            </div>
        </div>
    );
    ...
    return (
        <div className="relative">
            ...
            <button
                ...
                className="fixed bottom-6 right-6 bg-slate-900 text-white p-4 rounded-full shadow-2xl shadow-slate-400 hover:scale-110 transition-transform z-50 flex items-center justify-center border-4 border-slate-100"
            >
                {showAi ? <X size={24}/> : <Bot size={28}/>}
            </button>

            {/* AI Chat Window */}
            {showAi && (
                <div className="fixed bottom-24 right-6 w-80 bg-white rounded-2xl shadow-2xl border border-slate-100 z-50 p-4 animate-in slide-in-from-bottom-5 origin-bottom-right">
                    <div className="flex items-center gap-2 mb-3 pb-2 border-b">
                        <div className="bg-emerald-100 p-1.5 rounded-lg"><Sparkles className="text-emerald-600 w-4 h-4"/></div>
                        <span className="font-bold text-sm text-slate-800">小J 智慧管家</span>
                    </div>
                    <div className="h-64 bg-slate-50 rounded-xl p-4 text-xs text-slate-600 overflow-y-auto mb-3 custom-scrollbar">
                        <div className="flex gap-2 mb-4">
                             <div className="w-8 h-8 rounded-full bg-slate-200 flex-shrink-0 flex items-center justify-center">J</div>
                             <div className="bg-white p-3 rounded-2xl rounded-tl-none shadow-sm border border-slate-100">
                                 <p className="font-bold mb-1">江大哥您好！</p>
                                 <p>我是小J。目前您的「提案補助金」餘額充足，是否要前往許願樹投票？</p>
                             </div>
                        </div>
                    </div>
                    <div className="flex gap-2">
                        <button className="p-2.5 bg-slate-100 rounded-xl hover:bg-slate-200 transition-colors text-slate-600"><Mic size={18}/></button>
                        <input className="flex-1 bg-slate-100 rounded-xl px-4 py-2 text-xs outline-none focus:ring-2 focus:ring-emerald-500 transition-all" placeholder="輸入訊息..."/>
                        <button className="bg-emerald-600 text-white p-2.5 rounded-xl hover:bg-emerald-700 transition-colors"><Send size={18}/></button>
                    </div>
                </div>
            )}
        </div>
