...
"""
Chrome OS 客戶顯示器納管腳本
用途：將 Chrome OS 設備納管為客戶顯示器
說明：客戶顯示器就是 Chrome OS 設備，通過 device_purpose 標記用途
"""
...
def enroll_chrome_os_customer_display(
    device_name=None,
    ip_address=None,
    port=3477,
    mac_address=None,
    ...
    # 準備納管資料
    enrollment_data = {
        'device_id': f"CHROME_OS_{device_name.replace(' ', '_').upper() if device_name else 'CUSTOMER_DISPLAY'}",
        'device_name': device_name or f"Chrome OS Customer Display ({datetime.now().strftime('%H:%M:%S')})",
...
def main():
    parser = argparse.ArgumentParser(description='Chrome OS 客戶顯示器納管')
    ...
    print("=" * 60)
    print("  Chrome OS 客戶顯示器納管")
    print("=" * 60)
    print()
    print("說明：客戶顯示器就是 Chrome OS 設備")
    print("通過 device_purpose='customer_display' 標記用途")
    print()
    
    # 執行納管
    result = enroll_chrome_os_customer_display(
