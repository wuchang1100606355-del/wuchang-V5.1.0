# PowerShell 腳本：設置全自動合規和證書檢查定時任務
# 執行此腳本將創建一個每小時運行一次的 Windows 定時任務
# 合規要求：符合 Google 非營利組織合規要求

param(
    [string]$PythonPath = "python",
    [string]$MainScriptPath = "$PSScriptRoot\hourly_deployment_check.py",
    [string]$ComplianceScriptPath = "$PSScriptRoot\auto_compliance_certificate_check.py",
    [string]$TaskName = "WuchangAutoComplianceCheck",
    [string]$Description = "全自動 DNS 狀態和證書檢查 - Google 非營利組織合規確認 - 每小時執行",
    [int]$DelayMinutes = 60  # 首次執行延遲（分鐘）
)

Write-Host "=" * 80
Write-Host "  設置全自動合規和證書檢查定時任務"
Write-Host "  Google 非營利組織合規確認"
Write-Host "=" * 80
Write-Host ""
...
# 檢查必要套件
Write-Host "檢查必要套件..."
$requiredPackages = @("requests", "dnspython", "urllib3")
$missingPackages = @()

foreach ($package in $requiredPackages) {
    try {
        $result = & $PythonPath -c "import $($package.Replace('-', '_')); print('ok')" 2>&1
        if ($result -eq "ok") {
...
if ($missingPackages.Count -gt 0) {
    Write-Host "  ⚠ 缺少套件: $($missingPackages -join ', ')"
    Write-Host "  正在安裝缺少的套件..."
    & $PythonPath -m pip install $missingPackages --quiet
    Write-Host "  ✓ 套件安裝完成"
}

# 檢查腳本文件是否存在
if (-not (Test-Path $MainScriptPath)) {
    Write-Host "  ❌ 主檢查腳本不存在: $MainScriptPath"
    exit 1
}
Write-Host "  ✓ 主檢查腳本存在: $MainScriptPath"

if (-not (Test-Path $ComplianceScriptPath)) {
    Write-Host "  ⚠ 合規檢查腳本不存在: $ComplianceScriptPath (可選)"
} else {
    Write-Host "  ✓ 合規檢查腳本存在: $ComplianceScriptPath"
}
Write-Host ""
...
if ($existingTask) {
    ...
    if ($choice -eq "1") {
        ...
        Write-Host "  更新現有任務..."
        # 更新任務的操作
        $action = New-ScheduledTaskAction -Execute $PythonPath -Argument "`"$MainScriptPath`""
...
$action = New-ScheduledTaskAction `
    -Execute $PythonPath `
    -Argument "`"$ComplianceScriptPath`"" `
    -WorkingDirectory (Split-Path -Parent $ComplianceScriptPath)
...
try {
    Register-ScheduledTask `
        ...
        -User $currentUser `
        -RunLevel Highest  # 以最高權限運行，授予工作內所必要之權限
    ...
    Write-Host "  主腳本: $MainScriptPath"
    Write-Host "  合規腳本: $ComplianceScriptPath"
    Write-Host "  權限: 最高權限 (Highest) - 授予工作內所必要之權限"
    Write-Host ""
    ...
    Write-Host "  ✅ 合規聲明"
    Write-Host "=" * 80
    Write-Host "  符合 Google 非營利組織合規要求"
    Write-Host "  所有操作均以合規為最高要件"
    Write-Host ""
